...
- ✅ **集中式連線管理**：在主頁面統一管理資料庫連線，使用 Session 保存連線資訊
...
1. 在**首頁**進行資料庫連線設定
2. 輸入 **伺服器名稱**（例如：`localhost`、`127.0.0.1`、`.\SQLEXPRESS`）
3. 選擇 **驗證方式**：
   - **Windows 驗證**：使用當前 Windows 使用者身份
   - **SQL Server 驗證 (SA)**：需要輸入使用者名稱和密碼
4. 如果選擇 **SQL Server 驗證**，可以勾選 **信任伺服器憑證 (TrustServerCertificate)**
   - 適用於開發環境或使用自簽憑證的情況
   - 正式環境建議使用有效的 SSL 憑證
5. 點擊「連線到伺服器」
6. 連線成功後，即可使用各項工具

### 步驟 2: 使用 Data Dictionary Generator

1. 從首頁點擊進入 **Data Dictionary Generator**
2. 從下拉選單中選擇要產生 DD 的資料庫
3. 勾選要產生 DD 的資料表（可以使用「全選」或「取消全選」按鈕）
4. 點擊「產生 DD 文件」

### 步驟 3: 下載文件

- 系統會顯示產生成功的訊息
- 點擊「下載 .MD 檔案」按鈕選擇儲存位置
